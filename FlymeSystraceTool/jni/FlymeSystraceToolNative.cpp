...
static const char* g_fileToSaveContent = "";
...
static void dumpTrace()
{
    ...
    if (g_compress) {
    	...
    	char filename[256] = "/sdcard/";
		strcat(filename,g_fileToSaveContent);
		FILE *fp;
		fp=fopen(filename,"w");
		fclose(fp);
        ...
        while ((sent = sendfile(file_fd, traceFD, NULL, 64*1024*1024)) > 0);
        if (sent == -1) {
...
static void showHelp(const char *cmd)
{
    ...
    fprintf(stderr, "options include:\n"
    		        ...
    		        "  -f filename      filename is the file to save content"
            );
}
...
int main_function(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        ret = getopt_long(argc, argv, "f:a:b:ck:ns:t:z",
...
extern "C" {
    JNIEXPORT jstring JNICALL Java_com_example_flymesystracetool_FlymeSystraceToolActivity_stringFromJNI(JNIEnv* env,jobject thiz);
};
